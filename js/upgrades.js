    ...
    mass: {
        ...
        4: {
            ...
            effect(x) {
                ...
                let sp2 = 0.1
                let ss2 = E(5e12)
                let ret = step.add(1).mul(xx).add(1).softcap(ss,sp,0).softcap(1.8e2,0.02,0)
                if (hasTree("c1")) ret = step.add(1).mul(hasTree("c18")?tmp.supernova.tree_eff.c18:1).mul(xx).add(1).softcap(ss2,sp2,0)
                return {step: step, eff: ret, ss: ss}
            },
            effDesc(eff) {
                return {
