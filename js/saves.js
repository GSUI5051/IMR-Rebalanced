...
function calc(dt, dt_offline) {
    ...
    if (tmp.pass) {
        ...
        if (hasTree("qol1")) for (let x = 1; x <= tmp.elements.unl_length; x++) if (x<=tmp.elements.upg_length && x !== 118 && x < 119) ELEMENTS.buyUpg(x)
...
if (player.prestiges[1].gte(24) || player.prestiges[2].gte(1)) player.offline.active = false
